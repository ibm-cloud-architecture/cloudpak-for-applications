dojo.provide("ibm_atom.widget.FeedEntryEditor");
...
dojo.requireLocalization("ibm_atom.widget", "FeedEntryEditor");
dojo.requireLocalization("ibm_atom.widget", "PeopleEditor");
...
(function(){
	...
	try{
		...
		if(v){
			...
			v = parseFloat(v);
			if (v > 1.2) {
				dojo.deprecated("ibm_atom.widget.FeedEntryEditor", "Use dojox.atom.widget.FeedEntryEditor instead.");
			}
		}
	}catch(e){}
})();
...
dojo.declare(
	"ibm_atom.widget.FeedEntryEditor",
	ibm_atom.widget.FeedEntryViewer,
	{
    // summary:  An ATOM feed entry editor that allows viewing of the individual attributes of an entry.
	...
	templatePath: dojo.moduleUrl("ibm_atom", "widget/templates/FeedEntryEditor.html"),
	...
	postCreate: function() {
		...
		var _nlsResources = dojo.i18n.getLocalization("ibm_atom.widget", "FeedEntryEditor");
	...
	setEntry: function(entry, feed, leaveMenuState) {
		...
		ibm_atom.widget.FeedEntryEditor.superclass.setEntry.call(this, entry, feed);
	...
	_toggleEdit: function() {
		// summary: Internal function for toggling/enabling the display of edit mode
		// description: Internal function for toggling/enabling the display of edit mode
		//
		// returns:  Nothing.
		if (this._editable && this.enableEdit) {
			dojo.style(this.entryEditButton, 'display', 'none');
	...
	_handleEvent: function(entrySelectionEvent) {
		...
		ibm_atom.widget.FeedEntryEditor.superclass._handleEvent.call(this, entrySelectionEvent);
	},
	...
	_isEditable: function(entry) {
		...
		//
		// entry: object
		//   The ibm.io.atom.Entry object to examine
		//
		// returns:  Boolean denoting if the entry seems editable or not.. 
	...
	setTitle: function(titleAnchorNode, editMode, entry) {
		...
		//
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setTitle.call(this, titleAnchorNode, editMode, entry);
	...
	setAuthors: function(authorsAnchorNode, editMode, entry) {
		...
		//   The Feed Entry to work with.
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setAuthors.call(this, authorsAnchorNode, editMode, entry);
	...
	setContributors: function(contributorsAnchorNode, editMode, entry) {
		...
		//   The Feed Entry to work with.
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setContributors.call(this, contributorsAnchorNode, editMode, entry);
	...
	setId: function(idAnchorNode, editMode, entry) {
		...
		//   The Feed Entry to work with.
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setId.call(this, idAnchorNode, editMode, entry);
	...
	setUpdated: function(updatedAnchorNode, editMode, entry) {
		...
		//   The Feed Entry to work with.
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setUpdated.call(this, updatedAnchorNode, editMode, entry);
	...
	setSummary: function(summaryAnchorNode, editMode, entry) {
		...
		//   The Feed Entry to work with.
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setSummary.call(this, summaryAnchorNode, editMode, entry);
	...
	setContent: function(contentAnchorNode, editMode, entry) {
		...
		if (!editMode) {
			ibm_atom.widget.FeedEntryEditor.superclass.setContent.call(this, contentAnchorNode, editMode, entry);
	...
	_createEditor: function(anchorNode, node, multiline, rte) {
		// summary:  Function to create an appropriate text editor widget based on the given parameters.
		...
		//	 with a value parameter to a ibm_atom.io.atom.Content object.
		// multiline: boolean
		...
		if(!node) {
			...
			if(multiline) {
				...
				var viewNode = document.createElement("textarea");
				anchorNode.appendChild(viewNode);
				dojo.style(viewNode, 'width', '90%');
				var box = new dijit.form.SimpleTextarea({},viewNode);
			} else {
	...
	_switchEditor: function(event) {
		// summary:  Function to switch between editor types.
		...
		if(target.value === "text") {
			if(editor.declaredClass === "dijit.Editor") {
				...
				var value = editor.getValue(false);
	...
	_createPeopleEditor: function(anchorNode, node) {
		// summary: Creates a People Editor widget and returns it.
		// description: Creates a People Editor widget, sets its value, and returns it.
		//
		// anchorNode: Dom Node
		//		The node to attach the editor to.
		// node: Object
		//		An object containing the value to be put into the editor. Typically, this is an 
		//		ibm_atom.io.atom.Person object.
		//
		// returns: A new People Editor object.
		var viewNode = document.createElement("div");
		anchorNode.appendChild(viewNode);
		return new ibm_atom.widget.PeopleEditor(node,viewNode);
	},

	saveEdits: function() {
		// summary: Saves edits submitted when the 'save' button is pressed.
		...
		if(!this._new){
			...
			if(this._editors.title && (this._editors.title.getValue() != entry.title.value || this.entryTitleSelect.value != entry.title.type)) {
				value = this._editors.title.getValue();
				...
				entry.title = new ibm_atom.io.atom.Content("title", value, null, this.entryTitleSelect.value);
				modifiedEntry = true;
			}
			
			if(this._editors.id.getValue() != entry.id){
				entry.id = this._editors.id.getValue();
				modifiedEntry = true;
			}
			
			if(this._editors.summary && (this._editors.summary.getValue() != entry.summary.value || this.entrySummarySelect.value != entry.summary.type)) {
				value = this._editors.summary.getValue();
				if(this.entrySummarySelect.value === "xhtml") {
					value = this._enforceXhtml(value);
					if(value.indexOf('<div xmlns="http://www.w3.org/1999/xhtml">') != 0)
						value = '<div xmlns="http://www.w3.org/1999/xhtml">' + value + '</div>';
				}
				entry.summary = new ibm_atom.io.atom.Content("summary", value, null, this.entrySummarySelect.value);
				modifiedEntry = true;
			}
			
			if(this._editors.content && (this._editors.content.getValue() != entry.content.value || this.entryContentSelect.value != entry.content.type)) {
				value = this._editors.content.getValue();
				if(this.entryContentSelect.value === "xhtml") {
					value = this._enforceXhtml(value);
					if(value.indexOf('<div xmlns="http://www.w3.org/1999/xhtml">') != 0)
						value = '<div xmlns="http://www.w3.org/1999/xhtml">' + value + '</div>';
				}
				entry.content = new ibm_atom.io.atom.Content("content", value, null, this.entryContentSelect.value);
				modifiedEntry = true;
			}
			...
			if(this._editors.authors) {
				if(modifiedEntry) {
					entry.authors = [];
					var authors = this._editors.authors.getValues();
					for(i in authors)
			...
			if(this._editors.contributors) {
				if(modifiedEntry) {
					entry.contributors = [];
					var contributors = this._editors.contributors.getValues();
	...
	_handleSave: function(entry, location) {
		// summary: Function for handling the save of an entry, cleaning up the display after the edit is completed.
		// description: Function for handling the save of an entry, cleaning up the display after the edit is completed.
		//
		// entry: ibm_atom.io.atom.Entry object
	...
	cancelEdits: function() {
		// summary: Cancels edits and reverts the editor to its previous state (display mode)
		// description: Cancels edits and reverts the editor to its previous state (display mode)
		//
		// returns:  Nothing.
	...
	clear: function() {
		// summary: Clears the editor, destorys all editors, leaving the editor completely clear
		// description: Clears the editor, destorys all editors, leaving the editor completely clear
		this._editable=false;
		this.clearEditors();
		ibm_atom.widget.FeedEntryEditor.superclass.clear.apply(this);
	...
	_enforceXhtml: function(html) {
		// summary: Function for cleaning up/enforcing the XHTML standard in HTML returned from the editor2 widget.
		// description: Function for cleaning up/enforcing the XHTML standard in HTML returned from the editor2 widget.
		//
		// html: String
	...
	_closeTag: function(xhtml, tag) {
		// summary: Function for closing tags in a text of HTML/XHTML
		// description: Function for closing tags in a text of HTML/XHTML
		//
		// xhtml: String
	...
	_toggleNew: function() {
		// summary: Function to put the editor into a state to create a new entry.
		// description: Function to put the editor into a state to create a new entry.
	...
	_displaySections: function() {
		...
		ibm_atom.widget.FeedEntryEditor.superclass._displaySections.apply(this);
		
		// If we have editors to load after the nodes are created on the page, execute those now.
		if(this._toLoad) {
			for(i in this._toLoad){
				if(this._toLoad[i].generateEditor)
					var editor = dojo.hitch(this._toLoad[i], this._toLoad[i].generateEditor)();
				else
					var editor = this._toLoad[i];
...
dojo.declare(
	"ibm_atom.widget.PeopleEditor",
	[dijit._Widget, dijit._Templated, dijit._Container],
	{
		// summary: An editor for ibm_atom.io.atom.Person objects. 
		// description: An editor for ibm_atom.io.atom.Person objects.  Displays multiple rows for the respective arrays
		// 				of people.  Can add/remove rows on the fly.
		templatePath: dojo.moduleUrl("ibm_atom", "widget/templates/PeopleEditor.html"),
		...
		postCreate: function() {
			...
			var _nlsResources = dojo.i18n.getLocalization("ibm_atom.widget", "PeopleEditor");
			if(this.name)
				if(this.name == "Author")
		...
		destroy: function() {
			for(key in this._editors)
				for(key2 in this._editors[key])
					this._editors[key][key2].destroy();
			this._editors = [];
		},
		
		_createEditors: function(name, email, uri, index, widgetName){
			// summary: creates editor boxes (textbox widgets) for the individual values of a Person.
			// description: creates editor boxes (textbox widgets) for the individual values of a Person.
			//
			// name: String
		...
		_createEditor: function(value, id, name, labelNode, node) {
			// summary: Creates an individual editor widget (textbox) for a value.
			// description: Creates an individual editor widget (textbox) for a value.
			//
			// value: String
		...
		_removeEditor: function(event) {
			// summary: Removes a Person from our list of editors.
		...
		_add: function() {
			// summary: Adds a new block of blank editors to represent a Person.
			// description: Adds a new block of blank editors to represent a Person.
		...
		getValues: function() {
			// summary: Gets the values of this editor in an array.
			// description: Gets the values of this editor in an array, with each Person as an object within the array.
			//
			// returns: An array of anonymous objects representing ibm_atom.io.atom.Persons.
			var values = [];
			for(i in this._editors)
				if(this._editors[i])
					values.push({name: this._editors[i][0].getValue(), email: this._editors[i][1].getValue(), uri: this._editors[i][2].getValue()});
			return values;
		}
});