dojo.provide("ibm_atom.widget.FeedViewer");

dojo.require("dijit._Widget");
dojo.require("dijit._Templated");
dojo.require("dijit._Container");
dojo.require("ibm_atom.io.atom");
dojo.requireLocalization("ibm_atom.widget", "FeedViewerEntry");
...
(function(){
	...
	try{
		...
		if(v){
			...
			v = parseFloat(v);
			if (v > 1.2) {
				dojo.deprecated("ibm_atom.widget.FeedViewer", "Use dojox.atom.widget.FeedViewer instead.");
			}
		}
	}catch(e){}
})();
...
*/
dojo.declare(
	"ibm_atom.widget.FeedViewer",
	[dijit._Widget, dijit._Templated, dijit._Container],
	{

	// summary:  An ATOM feed viewer that allows for viewing a feed, deleting entries, and editing entries.
	...
	templatePath: dojo.moduleUrl("ibm_atom", "widget/templates/FeedViewer.html"),
	...
	postCreate: function() {
		// summary: The postCreate function.  
		...
		this.atomIO = new ibm_atom.io.atom.AtomIO();
		this.childWidgets = [];
	},
	
	startup: function() {
		// summary: The startup function.
		// description: The startup function.  Parses the filters and sets the feed based on the given url.
		this.containerNode = this.feedViewerTableBody;
		var children = this.getDescendants();
		for(var i in children){
			var child = children[i];
			if (child && child.isFilter) {
				this._includeFilters.push(new ibm_atom.widget.FeedViewer.CategoryIncludeFilter(child.scheme, child.term, child.label));
				child.destroy();
			}
		}
	...
	clear: function(){
		// summary: Function clearing all current entries in the feed view.
		// description: Function clearing all current entries in the feed view.
		//
		// returns:  Nothing. 
	...
	setFeedFromUrl: function(url){
		// summary: Function setting the feed from a URL which to get the feed.
		// description: Function setting the ibm.io.atom.Feed data into the view.
		//
		// url: string
	...
	setFeed: function(feed){
		// summary: Function setting the ibm.io.atom.Feed data into the view.
		// description: Function setting the ibm.io.atom.Feed data into the view.
		//
		// entry: object
		//   The ibm.io.atom.Feed object to process
		//
		// returns:  Nothing.
		this._feed = feed;
		this.clear();
	...
	_displayDateForEntry: function(entry){
		// summary: Internal function for determining the appropriate date to display.
		// description: Internal function for determining of a particular entry is editable.
		//
		// entry: object
		//   The ibm.io.atom.Entry object to examine.
		//
		// returns:  An appropriate date for the feed viewer display.
	...
	appendGrouping: function(titleText){
		// summary: Function for appending a new grouping of entries to the feed view.
		...
		var entryWidget = new ibm_atom.widget.FeedViewerGrouping({});
	...
	appendEntry: function(entry){
		// summary: Function for appending an entry to the feed view.
		// description: Function for appending an entry to the feed view.
		//
		// entry: object
		//   The ibm.io.atom.Entry object to append
		//
		// returns:  Nothing.

		var entryWidget = new ibm_atom.widget.FeedViewerEntry({"xmethod": this.xmethod});
	...
	_removeEntry: function(/*FeedViewerEntry*/ entry, /* boolean */success) {
		// summary: callback for when an entry is deleted from a feed.
		// description: callback for when an entry is deleted from a feed.
		...
		if(success) {
			...
			if( before.declaredClass === 'ibm_atom.widget.FeedViewerGrouping' && 
				(after === undefined || after.declaredClass === 'ibm_atom.widget.FeedViewerGrouping')){
				before.destroy();
			}
	...
	_rowSelected: function(evt){
		// summary: Internal function for handling the selection of feed entries.
		// description: Internal function for handling the selection of feed entries.
		//
		// evt: object
	...
	_deselectCurrentSelection: function() {
		// summary: Internal function for unselecting the current selection.
		// description: Internal function for unselecting the current selection.
		//
		// returns:  Nothing. 
	...
	onEntrySelected: function(entry){
		 ...
		 //
		 // entry: object
		 //   The ibm.io.atom.Entry object selected.
		 //
		 // returns:  Nothing. 
	},

	_isRelativeURL: function(url) {
		// summary: Method to determine if the URL is relative or absolute.
		...
		//		The URL to inspect.
		//
		// returns: boolean indicating whether it's a relative url or not.
	...
	_calculateBaseURL: function(fullURL, currentPageRelative) {
		// summary: Internal function to calculate a baseline URL from the provided full URL.
		// description: Internal function to calculate a baseline URL from the provided full URL.
		//
		// fullURL: string
		//	The full URL as a string.
		// currentPageRelative: boolean
		...
		//
		// returns: String of the baseline URL
		...
		if (fullURL !== null) {
			...
			if (currentPageRelative) {
				...
				if (index > 0) {
					...
					//console.debug("Protocol: " + protocol + "fragment: " + fragmentURL);
	...
	_isFilterAccepted: function(entry) {
		// summary: Internal function to do matching of category filters to widgets.
		// description: Internal function to do matching of category filters to widgets.
		//
		// returns: boolean denoting if this entry matched one of the accept filters.
	...
	addCategoryIncludeFilter: function(filter) {
		// summary: Function to add a filter for entry inclusion in the feed view.
		// description: Function to add a filter for entry inclusion in the feed view.
		// 
		// filter: object
		...
		if (filter) {
			...
			if (addIt) {
				this._includeFilters.push(ibm_atom.widget.FeedViewer.CategoryIncludeFilter(scheme, term, label));
			}
		}
	},

	removeCategoryIncludeFilter: function(filter) {
		// summary: Function to remove a filter for entry inclusion in the feed view.
		// description: Function to remove a filter for entry inclusion in the feed view.
		// 
		// filter: object
	...
	_addEntry: function(entry) {
		// summary: callback function used when adding an entry to the feed.
	...
	destroy: function() {
		// summary: Destroys this widget, including all descendants and subscriptions.
		// description: Destroys this widget, including all descendants and subscriptions.
		this.clear();
...
dojo.declare(
	"ibm_atom.widget.FeedViewerEntry",
	[dijit._Widget, dijit._Templated],
	{
	// summary: Widget for handling the display of an entry and specific events associated with it.
	// description: Widget for handling the display of an entry and specific events associated with it.

	templatePath: dojo.moduleUrl("ibm_atom", "widget/templates/FeedViewerEntry.html"),
	...
	postCreate: function() {
		var _nlsResources = dojo.i18n.getLocalization("ibm_atom.widget", "FeedViewerEntry");
		this.deleteButton.innerHTML = _nlsResources.deleteButton;
	},

	setTitle: function(text){
		// summary: Function to set the title of the entry.
		// description: Function to set the title of the entry.
		//
		// text: string
		//   The title.
		//
	...
	setTime: function(timeText){
		// summary: Function to set the time of the entry.
		// description: Function to set the time of the entry.
		//
		// timeText: string
	...
	enableDelete: function() {
		// summary: Function to enable the delete action on this entry.
		// description: Function to enable the delete action on this entry.
		//
		// returns:  Nothing. 
	...
	disableDelete: function() {
		// summary: Function to disable the delete action on this entry.
		// description: Function to disable the delete action on this entry.
		//
		// returns:  Nothing. 
		if (this.deleteButton !== null) {
			//TODO Fix this
	...
	deleteEntry: function(event) {
		// summary: Function to handle the delete event and delete the entry.
		// description: Function to handle the delete event and delete the entry.
		//
		// returns:  Nothing.
	...
	onClick: function(e){
		// summary: Attach point for when a row is clicked on.
		// description: Attach point for when a row is clicked on.
		//
		// e: Event object
...
dojo.declare(
	"ibm_atom.widget.FeedViewerGrouping",
	[dijit._Widget, dijit._Templated],
	{
	// summary: Grouping of feed entries.
	// description: Grouping of feed entries.
	templatePath: dojo.moduleUrl("ibm_atom", "widget/templates/FeedViewerGrouping.html"),
	
	groupingNode: null,  
	titleNode: null,

	setText: function(text){
		// summary: Sets the text to be shown above this grouping.
		// description: Sets the text to be shown above this grouping.
		//
		// text: String
		//		The text to show.
		if (this.titleNode.lastChild){this.titleNode.removeChild(this.titleNode.lastChild);}
...
dojo.declare(
	"ibm_atom.widget.AtomEntryCategoryFilter",
	[dijit._Widget, dijit._Templated],
	{
	// summary: A filter to be applied to the list of entries.
	// description: A filter to be applied to the list of entries.
	scheme: "",
	term: "",
	label: "",
	isFilter: true
...
dojo.declare(
	"ibm_atom.widget.FeedViewer.CategoryIncludeFilter",
	null,
	{

	constructor: function(scheme, term, label){
		// summary: The initializer function.
	...
	match: function(entry) {
		// summary: Function to determine if this category filter matches against a category on an atom entry
		// description: Function to determine if this category filter matches against a category on an atom entry
		//
		// returns:  boolean denoting if this category filter matched to this entry.
