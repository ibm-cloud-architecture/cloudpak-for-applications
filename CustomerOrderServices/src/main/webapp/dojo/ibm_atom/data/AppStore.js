dojo.provide("ibm_atom.data.AppStore");

dojo.require("ibm_atom.io.atom");
dojo.require("dojox.data.dom");
dojo.require("dojo.data.util.simpleFetch");
dojo.require("dojo.data.util.filter");
dojo.requireLocalization("ibm_atom.data", "messages");
...
(function(){
	...
	try{
		...
		if(v){
			...
			v = parseFloat(v);
			if (v > 1.2) {
				dojo.deprecated("ibm_atom.data.AppStore", "Use dojox.data.AppStore instead.");
			}
		}
	}catch(e){}
})();

dojo.declare("ibm_atom.data.AppStore",
	null,{
	...
	constructor: function(/*Object*/args){
		...
		if(args && args.url)
			this.url = args.url;
		if(!this.url) {
			var _nlsResources = dojo.i18n.getLocalization("ibm_atom.data", "messages");
			throw new Error(_nlsResources.missingUrl);
		}
	},
	
	_setFeed: function(feed, data) {
		// summary: Sets the internal feed using a ibm_atom.io.atom.Feed object.
		// description: Sets the internal feed using a ibm_atom.io.atom.Feed object.  Also adds
		...
		//
		// feed: ibm_atom.io.atom.Feed object
	...
	_getAllItems: function(){
		// summary: Function to return all entries in the Feed as an array of items.
		// description: Function to return all entries in the Feed as an array of items.
		//
		// returns: Array of all entries in the feed.
		var items = [];
		for(var i=0; i<this._feed.entries.length; i++)
	...
	_assertIsItem: function(/* item */ item){
		// summary: This function tests whether the item is an item.
	...
	_assertIsAttribute: function(/* String */ attribute){
		// summary: This function tests whether the item is an attribute.
		...
		for(var key in ibm_atom.io.atom._actions)
			if(key == attribute)
				return true;
		return false;
	},
	...
	_addUpdate: function(/* Object */ update){
		// summary: Internal function to add an updated entry to our updates array
		// description: Internal function to add an updated entry to our updates array
		//
		// update: ibm_atom.io.atom.Entry object
...
***************************************/
	...
	getValues: function(/* item */ item, 
		...
		if(flag) {
			...
			if(item[attribute]) {
				item = item[attribute];
				if(item.declaredClass == "ibm_atom.io.atom.Content")
					return [item.value];			
				return [item] ;
			}
		}
	...
	_fetchItems: function(request, fetchHandler, errorHandler) {
		...
		if(this._feed)
			...
			if(flag) {
				this._atomIO = new ibm_atom.io.atom.AtomIO();
		...
		for(var key in keywordArgs){
			if(this._assertIsAttribute(key)) {
				...
				switch(key){
					...
					case "issued":
					case "modified":
						entry[key] = ibm_atom.io.atom.util.createDate(value);
						break;
					case "content":
					case "summary":
					case "title":
					case "subtitle":
						entry[key] = new ibm_atom.io.atom.Content(key);
		...
		if(this._assertIsAttribute(attribute)) {
			switch(attribute){
				...
				case "modified":
					update[attribute] = item[attribute];
					this._addUpdate(update);
					item[attribute] = ibm_atom.io.atom.util.createDate(value);
					item.isDirty = true;
					return true;
				case "content":
				case "summary":
				...
				case "subtitle":
					update[attribute] = item[attribute];
					this._addUpdate(update);
					item[attribute] = new ibm_atom.io.atom.Content(attribute);
	...
	setValues: function(/* item */ item,
		...
		if(this._assertIsAttribute(attribute)) {
			switch(attribute){
				...
				case "modified":
					update[attribute] = item[attribute];
					item[attribute] = ibm_atom.io.atom.util.createDate(values[0]);
					item.isDirty = true;
					return true;
				case "content":
				case "summary":
...
dojo.extend(ibm_atom.data.AppStore,dojo.data.util.simpleFetch);